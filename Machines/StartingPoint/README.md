<h3>Разберём подробно каждую команду и используемые вместе с ней параметры</h3>
...
```--min-rate```=1000 фиксирует количество отправляемых пакетов в секунду не меньше заданного значения, в нашем случае оно равно 1000.

```-T4``` задает временной интервал для отправки пакетов на хост. Задается в интервале [1-5], чем больше число, тем быстрее скорость отправки. 
...
grep ^[0-9]
```
```grep``` выполняет поиск образца в текстовых файлах и выдает все строки, содержащие этот образец.
...
```-d, --delimiter```=РАЗДЕЛИТЕЛЬ задает РАЗДЕЛИТЕЛЬ колонок (полей) входных данных вместо символа табуляции. Применяется совместно с опцией -f.
...
```
Последняя строка ```49668``` в столбце очевидно тоже содержит управляющй символ переноса строки, и в результате замены он был изменен на знак запятой. Для того, чтобы запрос
